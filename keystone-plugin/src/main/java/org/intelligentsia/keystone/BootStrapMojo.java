...
public class BootStrapMojo extends AbstractMojo {
    ...
    /**
     * Set of scope that dependencies must match in order to be included in final archive.
     * @parameter alias="${scopes}"
     */
     ...
     *
     * @param root root node where dependencies analysis start.
     * @return library directory
     * @throws MojoExecutionException
     */
        ...
        try {
            ...
            final ArtifactFilter artifactFilter = new ScopeArtifactFilter(DefaultArtifact.SCOPE_COMPILE) {
                ...
                public boolean include(Artifact artifact) {
                    // test with given scope if any
                    if (scopes!=null) {
                        return scopes.contains(artifact);
                    }
                    // include all scope except "test".
            ...
            for (final Iterator<?> iterator = rootNode.getChildren().iterator(); iterator.hasNext(); ) {
                ...
                collect(child, artifacts, artifactFilter);
            }
     ...
     *
     * @param root starting point node
     * @param artifacts artifact set that match the filter
     * @param artifactFilter artifact Filter
     */
    private void collect(final DependencyNode root, final Set<Artifact> artifacts, ArtifactFilter artifactFilter) {
